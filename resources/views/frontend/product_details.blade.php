                        ...
                        <div class="text-left">
                            ...
                            <div class="row align-items-center">
                                <div class="col-auto">
                                    ...
                                    <span class="bg-soft-primary text-primary px-3 py-1 rounded fw-600"><a href="{{ route('shop.visit', $detailedProduct->user->shop->slug) }}"
                                            class="text-reset">{{ $detailedProduct->user->shop->name }}</a></span>
                                    @else
                                        <span class="bg-soft-primary text-primary px-3 py-1 rounded fw-600">{{ translate('Inhouse product') }}</span> 
                                    @endif
                                </div>
                                ...
                                @endif
                                    ...
                                    <!-- <div class="col-auto">
                                        <button class="btn btn-sm btn-success"
                                            onclick="show_contact_modal()">{{ translate('Contact Seller') }}</button>
                                    </div> -->
                                        ...
                                        <div class="product-quantity d-flex align-items-center">
                                            ...
                                            <div class="avialable-amount opacity-60">
                                                @if ($detailedProduct->stock_visibility_state == 'quantity')
                                                    <span class="bg-soft-dark rounded px-3 py-1 text-dark fw-600"> <span id="available-quantity">{{ $qty }}</span> {{ translate('available') }} </span>
                            ...
                            <div class="mt-3">
                                ...
                                @else
                                    ...
                                    <button type="button" class="btn btn-sm btn-success mr-2 buy-now fw-600" style="padding: 10px 30px;"
                                            onclick="show_contact_modal()"><i class="las la-comments fs-2tx"></i> {{ translate('Contact Seller') }}</button>
                                    <button type="button" class="btn btn-soft-primary buy-now fw-600" onclick="buyNow()">
...
@section('modal')
    ...
    <div class="modal fade" id="contact_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
            <div class="modal-content position-relative">
                <div class="modal-header">
                    <h5 class="modal-title fw-600 h5">Contact Seller</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form class="" action="{{ route('conversations.store') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body gry-bg px-6 pt-3">
                        <div>
                            <h6 class="py-2 m-0">Contact Detail:</h6>
                            @if($detailedProduct->user->shop)
                                <h1 class="fw-600"> 0{{ $detailedProduct->user->shop->phone }} <button class="btn btn-sm btn-soft-success rounded" onclick="copyNumber({{ $detailedProduct->user->shop->phone }})"  type="button"><i class="las la-file"></i>Copy</button></h1>
                            @else
                                <h1 class="fw-600"> {{ env('DEFAULT_NUMBER', '07066293982') }} <button class="btn btn-sm btn-soft-success rounded" onclick="copyNumber({{ env('DEFAULT_NUMBER', '07066293982') }})"  type="button"><i class="las la-file"></i>Copy</button></h1>
                            @endif
                        </div>
                        <div class="row py-1 mt-4">
                            <div class="col-md-6 col-sm-12">
                                <button class="m-1 d-flex align-items-center px-1 bg-soft-success border-0" style="border-radius: 40px; width: max-content;">
                                    <img style="width: 40px;" src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/5e/WhatsApp_icon.png/598px-WhatsApp_icon.png" alt="...">
                                    <p style="margin: revert;" class="px-2">WhatsApp Message</p>
                                </button>
                            </div>
                            <div class="col-md-6 col-sm-12">
                                <button class="m-1 d-flex align-items-center px-1 bg-soft-dark border-0" style="border-radius: 40px; width: max-content;">
                                    <img style="width: 40px;" src="https://static-00.iconduck.com/assets.00/call-icon-2048x2048-9fglrllu.png" alt="...">
                                    <p style="margin: revert;" class="px-2">Phone Call</p>
                                </button>
                            </div>
                        </div>
                        <div class="pt-3">
                            ...
                            <div>
                                ...
                                <input type="text" class="form-control bg-soft-dark" placeholder="{{ $detailedProduct->name }}...">
                            </div>
                            <div class="py-2">
                                <label for="message">Message: </label>
                                <textarea class="form-control bg-soft-dark" placeholder="Enter your message..." name="message" id="" cols="30" rows="10"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-danger fw-600"
                            data-dismiss="modal">{{ translate('Cancel') }}</button>
                        <button type="submit" class="btn btn-success fw-600">{{ translate('Send') }}</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('script')
    <script type="text/javascript">
        ...
        function copyNumber(numb) {
            var num = numb;
            // Create a temporary textarea element to hold the phone number
            var tempTextArea = document.createElement("textarea");
            tempTextArea.value = num;
            ...
            // Use the newer and more reliable method to copy to clipboard
            navigator.clipboard.writeText(num).then(function() {
                AIZ.plugins.notify('success', 'Phone number copied 👍');
            }).catch(function(err) {
